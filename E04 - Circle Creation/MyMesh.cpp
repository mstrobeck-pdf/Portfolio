...
void MyMesh::GenerateCircle(float a_fRadius, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	*/
	...
	//Solution created based on A02 homework, generateCylinder and also solution to this exercise
	//I simply could not figure out how to increas the number needed to make a circle, but I understood the code given to us for some reason
	...
	{
		//create a temporary vector3 that is equal to the following equation
		vector3 temp = vector3(cos(theta) * a_fRadius, sin(theta) * a_fRadius, 0.0f);
		
		//multiply by two makes a star
		//theta += delta * 2;
		theta += delta;

		//push back the temporary value to make room for another
		vertex.push_back(temp);
	}

	int i = 0;
	while (i < a_nSubdivisions) {
		//adding three to vertex[(i + 3) % ..] leads to creation of a star here
		AddTri(ZERO_V3, vertex[i], vertex[(i + 1) % a_nSubdivisions]);
		i++;
	}

	/*
	Originally had the two values reversed but that placed the circle facing the wrong direction
